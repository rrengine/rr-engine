...
def get_current_user(
    ...
    # Set per-request session variables for Postgres Row Level Security (RLS).
    # These settings are read by DB policies via current_setting('app.user_id').
    # Safe no-op if RLS is not enabled yet.
    try:
        db.execute(text("SELECT set_config('app.user_id', :uid, true)"), {"uid": str(user.id)})
    except Exception:
        # Don't block MVP auth if DB is not Postgres or RLS not installed.
        pass
    return user
