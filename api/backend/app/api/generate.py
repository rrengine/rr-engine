...
def generate(payload: GenerateRequest, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    # Deterministic geometry placeholder + reproducibility key.
    ensure_geometry_assets(db, new_gen.id)
    ...
    return GenerateResult(
        ...
        message="Generation created. Geometry build is separate and deterministic (next step).",
    )
